...
class MultimodalClient extends AudioClient {
    ...
    async initializeWebcam(videoElement) {
        ...
        try {
            ...
            console.log('🎥 WEBCAM ACTIVATED - Video stream initialized successfully');
    ...
    async initializeScreenShare(videoElement) {
        ...
        try {
            ...
            console.log('🖥️ SCREEN SHARE ACTIVATED - Screen sharing initialized successfully');
    ...
    startVideoStream(frameRate = 1) {
        ...
        console.log(`📹 VIDEO STREAMING STARTED - Sending ${this.videoMode} frames at ${frameRate} FPS`);
    ...
    stopVideoStream() {
        if (this.videoSendInterval) {
            clearInterval(this.videoSendInterval);
            this.videoSendInterval = null;
            console.log('⏹️ VIDEO STREAMING STOPPED - Frame transmission halted');
        }
    }
    ...
    stopVideo() {
        ...
        const wasActive = this.isVideoActive;
        ...
        if (wasActive) {
            const modeIcon = previousMode === 'screen' ? '🖥️' : '🎥';
            const modeText = previousMode === 'screen' ? 'SCREEN SHARE' : 'WEBCAM';
            console.log(`${modeIcon} ${modeText} DEACTIVATED - Video stream stopped and resources cleaned up`);
        }
    }
    ...
    // Override audio methods to add logging
    async startRecording() {
        const result = await super.startRecording();
        if (result) {
            console.log('🔴 AUDIO RECORDING STARTED - Microphone is now capturing audio');
        }
        return result;
    }
    ...
    stopRecording() {
        super.stopRecording();
        console.log('⏹️ AUDIO RECORDING STOPPED - Microphone capture halted');
    }
    ...
    close() {
        this.stopVideoStream();
        this.stopVideo();
        console.log('🔌 MULTIMODAL CLIENT CLOSED - All audio and video resources cleaned up');
        super.close();
    }
}
