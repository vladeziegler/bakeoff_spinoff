...
class PCMPlayerProcessor extends AudioWorkletProcessor {
    constructor(options) {
        ...
        this.log = (...args) => {
            if (this.verbose) {
                console.log('[AudioWorklet] [Player]', ...args);
            }
        };

        this.log('PCMPlayerProcessor constructor called');
        ...
        this.port.onmessage = (event) => {
            this.log('Received message:', typeof event.data);
            ...
            if (event.data && event.data.command === 'endOfAudio') {
                ...
                this.log("endOfAudio received, clearing the buffer.");
                return;
            }

            // Check if the data is an ArrayBuffer
            if (event.data instanceof ArrayBuffer) {
                this.log(`Processing ${event.data.byteLength} bytes of audio data`);

                // Create a view of the ArrayBuffer as Int16 samples
                const int16Samples = new Int16Array(event.data);
                this.log(`Converted to ${int16Samples.length} int16 samples`);
        ...
        this.log('PCMPlayerProcessor initialized');
    }
    ...
    process(inputs, outputs, parameters) {
        ...
        if (bufferSize > 0) {
            this.log(`Playing audio! Buffer size: ${bufferSize}, processing ${channel.length} samples`);
        }
        ...
        for (let i = 0; i < channel.length; i++) {
            if (this.readIndex !== this.writeIndex) {
                ...
                if (!this.notifiedFinished) {
                    this.log('Playback finished, buffer empty');
        ...
        if (samplesWritten > 0) {
            this.log(`Output ${samplesWritten} samples to audio system`);
        }
